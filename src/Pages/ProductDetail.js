...
function ProductDetail() {
  ...
  function handleRam(idRam) {
    fetch(
      `http://localhost:8080/admin/getProducts/${product.dataProductDetail.name}/${idRam}`
    )
      .then((response) => response.json())
      .then((data) => {
        const arr = data[0];
  ...
  function handleColor(idColor) {
    fetch(
      `http://localhost:8080/admin/getProductsColor/${product.dataProductDetail.name}/${idColor}`
    )
      .then((response) => response.json())
      .then((data) => {
        const arr = data[0];
        ...
        <div className="product-detail">
          <div className="container-carousel">
            <div className="row">
              <div className="col-md-12">
                {product && product.dataProductDetail ? (
                  product.dataProductDetail.images &&
                  product.dataProductDetail.images.length > 0 ? (
                    <div
                      id="custCarousel"
                    ...
                    >
                      <div className="carousel-inner">
                        {product.dataProductDetail.images.map(
                          (product, index) => {
                            if (index == 0) {
                              return (
                                <div
          ...
          <div className="product-info ml-md-4">
            <h2 className="prod-name">
              {product && product.dataProductDetail
                ? product.dataProductDetail.name
            ...
            <div className="price">
              <span className="price-new">
                {product && product.dataProductDetail
                  ? product.dataProductDetail.priceNew
                  : "Giá chưa được cập nhật"}
              </span>
              <span className="discount">
                {product && product.dataProductDetail
                  ? product.dataProductDetail.priceOld
                  : "Giá chưa được cập nhật"}
              </span>
              <span className="percent">
                ...
                product.dataProductDetail.discounts.length > 0
                  ? product.dataProductDetail.discounts[0].product
                      .percentDiscount + "%"
                  : ""}
              </span>
            </div>
            ...
            <ul className="prods-group">
              {product && product.storages
                ? product.storages.map((item, index) => (
                    <li
                      onClick={() => handleRam(item.id)}
                      style={
                        item.id == product.dataProductDetail.storage.id
                          ? { backgroundColor: "blue" }
                          : { backgroundColor: "black" }
                      }
                      key={index}
                      className="merge__item item"
                    >
                      <span>
            ...
            <div>
              {product && product.colors
                ? product.colors.map((item, index) => (
                    <span
                      style={
                        item.id == product.dataProductDetail.color.id
                          ? { backgroundColor: "blue" }
                          : { backgroundColor: "black" }
                      }
              ...
              <button>
                <a
                  ...
                  href={
                    user &&
                    product.dataProductDetail &&
                    product.dataProductDetail.id
                      ? `/Cancel/${product.dataProductDetail.id}`
                      : user && product.productId
                      ? `/Cancel/${product.productId}`
                      : "/NoLogin/1"
                  }
                >
                  Mua ngay
            ...
            <ul className="describe">
              <li>
                <i className="fas fa-box"></i>Bộ sản phẩm gồm: Hộp, Sách hướng
                dẫn, Cây lấy sim, Cáp Type C
              </li>
              <li>
                <i className="fas fa-undo"></i>Hư gì đổi nấy 12 tháng tại 3453
                siêu thị trên toàn quốc Xem chi tiết chính sách bảo hành, đổi
                trả
              </li>
              <li>
                <i className="fas fa-shield-alt"></i>Bảo hành chính hãng 1 năm
              </li>
              <li>
                <i className="fas fa-shipping-fast"></i>Giao hàng nhanh toàn
                quốc Xem chi tiết
              </li>
              <li>
                <i className="fas fa-phone-alt"></i>Tổng đài: 1900.9696.42 (9h00
                - 21h00 mỗi ngày)
              </li>
            </ul>
          </div>
        ...
        <div className="container">
          <div className="row mh-100">
            <div className="col-12 mh-100 tab-content-3">
              <ul
                className="nav nav-tabs d-flex justify-content-center tab-content-l"
                role="tablist"
              >
              ...
              <div className="tab-content mh-100">
                    ...
                    <div className="col-lg-4 abc">
                      <div className="card">
                        <div className="card-block">
                          <h4>Card</h4>
                          <p>
                            Meggings flannel Brooklyn literally small batch,
                            mumblecore PBR try-hard kale chips. Brooklyn vinyl.
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div
                  role="tabpanel"
                  className="tab-pane py-3 mh-100"
                  id="profile"
                >
                  <div className="row flex-row justify-content-center">
                    <div className="col-lg-4 abc">
            ...
            <div className="be-comment-block">
              ...
              {product && product.commentList
                ? product.commentList.map((item, index) => (
                    <div key={index} className="be-comment">
                      ...
                      <div className="be-comment-content">
                        ...
                        <p className="be-comment-text">{item.content}</p>
                        <div>
                          <button
